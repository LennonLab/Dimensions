...
active.comm <- Pond97[grep("*-cDNA", rownames(PondREL)), ]
...
all.comm <- Pond97[grep("*-DNA", rownames(PondREL)), ]
...
env.dat <- as.matrix(subset(env, select = ORP:DON))
env.chem <- env.dat
# Conduct CCA 
...
active.dist <- log10(1 - vegdist(active.comm, method=metric, binary=F))
all.dist <- log10(1 - vegdist(all.comm, method=metric, binary=F))
...
plot(geo.dist, all.dist, xaxt = "s", las = 1, 
   ylab="log(similarity)", xlab="Distance, km", col = "azure3", 
   cex.lab=1.5)
...
plot(all.cca1.dist, all.dist, xaxt = "s", las = 1, 
   ylab="log(similarity)", xlab="CCA1", col = "azure3",
   cex.lab=1.5)
