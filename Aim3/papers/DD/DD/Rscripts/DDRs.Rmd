...
all.cca <- vegan::cca(all.sub.comm ~ env.chem)
...
slope <- round(coefficients(DD.all.cca1)[2], 3)
p <- round(summary(DD.all.cca1)$coefficients[8], 3)
plot(all.cca1.dist, all.dist, xaxt = "s", las = 1, 
   ylab="log(similarity)", xlab="CCA1", col = "azure3", cex.lab=1.5)
abline(DD.all.cca1, col = "red")
legend("bottomleft", paste("All (weighted)\nslope=",slope," p = ",p), bty="n", cex=1.2)


# Make Plot for active cca1
slope <- round(coefficients(DD.active.cca1)[2], 3)
p <- round(summary(DD.active.cca1)$coefficients[8],3)
plot(active.cca1.dist, active.dist, xaxt = "s", las = 1, 
   ylab="log(similarity)", xlab="Distance, km",col = "azure3",cex.lab=1.5)
abline(DD.active.cca1, col = "red")
legend("bottomleft", paste("Active (weighted)\nslope=",slope," p = ",p), bty="n", cex=1.2)
