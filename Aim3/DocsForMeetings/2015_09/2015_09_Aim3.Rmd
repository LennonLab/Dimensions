---
title: "Spatial vs. temporal storage: The dual role of microbial seed banks in driving geographical patterns of microbial diversity"
...
## Overview
Themes: Spatial vs. temporal storage and/or local vs. regional drivers

General hypothesis: Seed banks contribute to homogenizing dispersal, which helps explain why microbial TARs often have shallow slopes. 
But note: Zinger et al. 2012 reveal slopes similar to those of plants and animals.

Initial DoB Prediction: Steeper slopes (i.e. higher z-values, greater scaling exponent) for the active community.
Initial and repeated result: Our data don't support this hypothesis.
Note: We see little homogeneity all-around, when looking only at presence/absence. This is driven by rare members, as weighted similarity measures greatly increase apparent compositional similarity between sites.

Potential Explanation: 
Our initial hypothesis focused on the spatial influence of dormancy. 
It did not consider temporal storage effect. Activity and seed bank dynamics could be controlled by regional cues and age of the seed bank.

Recent result that Ken is looking into:
The dominant members differ greatly between 'active' and 'all'. This deserves more analyses and it would be great to know if there's something known about the dominant taxa.
...
### D. Import data and prepare data for analyses
```{r, results = 'hide', echo=FALSE, message = FALSE, warning = FALSE}
...
### 1.) Dependence of the taxa-area relationship on the number (detection) of rare taxa
The species-area relationshp describes the rate at which species are discovred with increasing area. The SAR one of ecology's oldest and most intensively studied patterns. Arrhenius (1921) first described the general form of the *species-area relationship (SAR)* as a power-law: $S = cA^{z}$ where S is species richnness and A is area. Arrhenius's formula predicts a rate of increase in richness that is approximately linear in log-log space. That is, $log(S) = c + zlog(A)$, where z is the scaling exponent.
...
for (cutoff in cutoffs){
  ...
  # all, distance
  sar <- as.data.frame(SAR.accum.dist(all.sub.comm, coord.dist.ls$geo.dist, iterations))
  OLS <- lm(sar$V2 ~ sar$V1)
  all.zs.dist <- c(all.zs.dist, coefficients(OLS)[2])
  
  # active, distance
  sar <- as.data.frame(SAR.accum.dist(active.sub.comm, coord.dist.ls$geo.dist, iterations))
  OLS <- lm(sar$V2 ~ sar$V1)
...
z.lists <- as.data.frame(c(all.zs.dist, active.zs.dist, all.zs.rand, active.zs.rand))
...
# plot on same grid, each series colored differently -- good if the series have same scale
...
# ggplot(z.df, aes(cutoff,value)) + geom_line() + facet_grid(series ~ .)
```
...
# Set Initial Plot Parameters
par(mfrow=c(1, 1))#, mar = c(5, 4, 4, 2) + 0.1, oma =c(0,0,0,0))
heatmap.2(as.matrix(active.comm.dom), distfun = function(x) vegdist(x, method = "bray"),
          hclustfun = function(x) hclust(x, method = "ward.D2"), 
          col = jet.colors(100), trace = "none", density.info = "none",
...
all.comm.dom <- log10(all.comm[, 1:10]+1)
active.comm.dom <- log10(active.comm[, 1:10]+1)
comm.dat <- active.comm.dom

# Geographic variables
geo.dat <- as.matrix(subset(env, select = lat:long))
#rda.geo <- rda(comm.dat ~ geo.dat)

phys.dat <- as.matrix(subset(env, select = Diameter:Cal_Volume))

# Physicochemical variables
chem.dat <- as.matrix(subset(env, select = ORP:DON))
#rda.phys <- rda(comm.dat ~ phychem.dat)

# Two explanatory matrices -- Hellinger-transform Y
mod <- varpart(Y=comm.dat, X=geo.dat, chem.dat, transfo="hellinger")
mod
plot(mod)
...
#geo.anova
#phychem.anova <- anova(rda.phys, step=200, perm.max=200)
#phychem.anova
