...
SAR.accum.dist <- function(com){
  mymatrix <- data.matrix(coord.dist.ls)
  mymatrix <- as.data.frame(mymatrix)
  Alist <- c()
  Slist <- c()
  
  num.ponds <- c(1,2,3,4,6,8,12,16,24,32,42,50)
  for (i in num.ponds) {   
    areas <- c() # hold iterated area values 
    Ss <- c() # hold iterated S values
    ...
    for(j in 1:10){
      used <- c(0)
      area1 <- 0
      cum.abs <- as.numeric(vector(length = length(com[1, ])))
      ...
      for (k in 1:i) { # Loop through ponds
        if (k == 1) {
          pondID <- sample(51, size = 1)
            ...
            sdata <- subset(mymatrix, FALSE == is.element(NBX, used) & FALSE == is.element(NBY, used))
            sdata <- subset(sdata, NBX == pondID | NBY == pondID)
            sdata <- subset(sdata, geo.dist == min(sdata[, 3]))
            sdata <- data.matrix(sdata)
            print(sdata)
            
            used <- c(used, pondID)
            area1 <- area1 + pond.areas[pondID] # aggregating area
