  ...
  for (i in num.ponds) {   
    ...
    for(j in 1:1000){
      ...
      for (k in 1:i) { # Loop through ponds
        ...
        sdata <- subset(coord.dist.ls, NBX == pondID | NBY == pondID)
        sdata <- subset(sdata, geo.dist == min(sdata[, 3]))
        
        if (sdata[1] == pondID) {
          pondID <- as.integer(sdata[2])
          } else {
            pondID <- as.integer(sdata[1])
        }
        
        used <- c(used, pondID)
        area <- area + pond.areas[pondID] # aggregating area
